...
public class DbQueryRequest implements IServerRequest {
    ...
    public String logString() {
    	String order = (orderBy == null ? null : 
    			String.format("{\"field\":\"%s\", \"mode\"=\"%s\"}", 
    					orderBy.field, orderBy.mode.toString()));

    	String jstring = "{" + 
        		String.format("\"reqClass\":\"%s\",", this.getClass().getName());
    	
    	if (database != null)
        		jstring += String.format("\"database\":\"%s\",", database);

    	if (entityClass != null)
    		jstring += String.format("\"entityClass\":\"%s\",", entityClass);

    	jstring += String.format("\"fields\":\"%s\",", fields) +
    			String.format("\"distinct\":%s,", distinct) +
    			String.format("\"constraints\":\"%s\"", constraints);

   		if (order != null)
    		jstring += String.format(",\"orderBy\":%s", order);

    	if (limit != null)
    		jstring += String.format(",\"limit\":%d", limit);
